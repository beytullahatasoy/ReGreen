...
## 3. Model Girdisi — 6 Öznitelik (Random Forest'a girer)
...
Bu 6 alan **hücrenin sabit özelliğidir** — model tekrar çalışsa bile değişmez, bu yüzden DB'de `Predictions` değil `Cells` tablosunda tutulur.
...
| `ndvi_drop`      | float         | Hayır                                                                 | **Üç rollü:** (1) modele girmez, (2) `prediction_status` kural motorunda eşik değişkeni (§6.1), (3) UI'da gösterilir | Gözlenen: **−0,12 – 0,74**. Nadiren negatif olabilir (yangın sonrası ölçümde NDVI'nin hafifçe yüksek çıkması — ölçüm zamanlaması/gürültü kaynaklı, hata değil)                                                                                             | `0.272803`   | `Cells.NdviDrop`      |
| `severity_class` | string (enum) | Hayır                                                                 | SADECE gösterim                                                                                                      | **4 değer** (53 yangının TAMAMI tarandı): `dusuk`, `orta-dusuk`, `orta-yuksek`, `yuksek`. ⚠️ `sample-data/frontend-data/HUCRE_PANELI.md` bu alanı yanlışlıkla 3 değerliymiş gibi (`dusuk`/`orta`/`yuksek`) tanımlıyor — bağlayıcı olan buradaki 4 değerdir | `orta-dusuk` | `Cells.SeverityClass` |
...
**Terminoloji netliği:** `prediction_status`, Random Forest'ın çıktısı DEĞİLDİR — dNBR + NDVI eşiğine dayanan bir **kural motoru** kararıdır. Random Forest'ın gerçek tahmini sadece `recovery_gap_pred`'dir.
...
| ------------------- | --------------------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------- |
| `recovery_gap_pred` | 0,1187 – 0,576                          | `-0.5 ≤ x ≤ 1.5` dışını reddet, logla | **Yön: yüksek = KÖTÜ** (eski hâline dönememiş). `predicted` dışında HER ZAMAN null |
| `priority_score`    | 0,0 – 0,9683                            | `Math.Clamp(x, 0, 1)`                 | Teorik aralık 0–1, bu paketteki gerçek maksimum 0,9683                             |
...
Her `{fire_id}_metadata.json`'da `normalization_reference` alanı var (`recovery_gap_pred`, `slope_deg`, `road_distance_km` için min/max). Gerçek örnek (`AKD_2021_05_metadata.json`):

```json
"normalization_reference": {
  "recovery_gap_pred": { "min": 0.1796, "max": 0.4305 },
...
19 sütun, sırası ve sayısı 53 dosyanın tamamında birebir aynı — doğrulandı. Örnek satır (`AKD_2021_05_hucreler.csv`, `predicted` durumunda, tüm alanları dolu):
...
| `generated_at`                                          | datetime (ISO 8601) | Hayır | Paketin üretildiği an                                                                                                                                                                                                | `"2026-08-19T17:22:55+00:00"`                                                      |
| `model_version`                                         | string              | Hayır | ör. `rf_v1` — `ModelRuns` tablosuna gider                                                                                                                                                                            | `"rf_v1"`                                                                          |
...
> **Önceki sürümde eksikti:** `project`, `generated_at`, `model_version`, `schema_version`, `cell_size_m`, `crs`, üst seviye `priority_weights`/`priority_thresholds`, ve `fires[]` altındaki `fire_date`/`province`/`region`/`cell_count`/`burned_area_ha`/`has_perimeter` hiç belgelenmemişti. 1.2 ile eklendi.
...
| `model_version`                              | string              | Hayır    | ör. `rf_v1`. `ModelRuns` tablosuna gider                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Bu pakette sabit `"rf_v1"`                                                    | `"rf_v1"`                                                                                                          |
| `generated_at`                               | datetime (ISO 8601) | Hayır    | `ModelRuns` tablosuna gider                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | ISO 8601, UTC offsetli                                                        | `"2026-08-19T17:22:55+00:00"`                                                                                      |
...
## 11. DB Şeması Özeti (referans: Teknik Mimari v4.1 §7)

| Tablo         | İçerik                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Fires`       | Yangın kimliği, perimeter, marker (representative_point), quality_flag/note, `modis_area_ha`/`burned_area_ha` (§2.1) — bu ikisi PDF v4.1 DDL'inde zaten `Fires` tablosunda mevcut. `has_perimeter` İSE DDL'de yok (mevcut veride 53/53 `true` olduğu için `PerimeterGeoJson NOT NULL` yeterli varsayılmış) — **PDF v4.2 ile eklenecek hedef alan**, şu an DB'de karşılığı yok                                                                                                                                                                                                                                     |
| `Cells`       | Hücre merkezi + TÜM sabit özellikler (§3, §4) — CSV silinse bile veri kaybolmaz                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `ModelRuns`   | **Her yangın + teslimat kombinasyonu bir satır** (PDF v4.1 §7 DDL: `ModelRuns.FireId` FK + `UNIQUE(FireId, ModelVersion, GeneratedAt)`) — `normalization_reference` fire'a özel olduğu için tek bir AI teslimatı (53 yangın) 53 ayrı `ModelRuns` satırı üretir, `ModelVersion`+`GeneratedAt` bu 53 satırı aynı teslimata bağlar. `normalization_reference`, varsayılan ağırlık, eşikler, `schema_version` PDF v4.1 DDL'inde ZATEN burada donuyor. `in_training_set`/`out_of_fold_cells` İSE DDL'de yok (§9.3, §12/v1.3 notu) — **PDF v4.2 ile `ModelRuns`'a eklenecek hedef kolonlar**, şu an DB'de karşılığı yok |
...
- `sample-data/frontend-data/HUCRE_PANELI.md` — hücre detay paneli spesifikasyonu (⚠️ `severity_class` için 3 değerli eski varsayım içeriyor, bu belgedeki 4 değerli liste esas alınmalı)
